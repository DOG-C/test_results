...
def text_to_image(text, output_path, font_path='C:\\Windows\\Fonts\\Consola.ttf', font_size=10):
    # 根据文本长度和字体大小估算所需的图像尺寸
    lines = text.split('\n')
    max_line_length = max(len(line) for line in lines)
    image_width = max_line_length * font_size
    image_height = len(lines) * font_size * 1.5  # 为行间距留出空间
    ...
    for line in lines:
        ...
        y_text += font_size * 1.5  # 更新下一行的y坐标
